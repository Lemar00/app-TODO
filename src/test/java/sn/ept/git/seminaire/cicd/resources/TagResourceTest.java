...
class TagResourceTest extends BasicResourceTest {
    ...
    void delete_withInvalidId_shouldReturnNotFound() throws Exception {
        ...
        mockMvc.perform(delete(UrlMapping.Tag.DELETE, invalidId.toString())
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound());
    }
    ...
    @Test
    void update_withNonExistentTag_shouldReturnNotFound() throws Exception {
        UUID nonExistentId = UUID.randomUUID();
        TagVM updatedVm = TagVMTestData.defaultVM();

        mockMvc.perform(
                        put(UrlMapping.Tag.UPDATE, nonExistentId)
                                .contentType(MediaType.APPLICATION_JSON)
