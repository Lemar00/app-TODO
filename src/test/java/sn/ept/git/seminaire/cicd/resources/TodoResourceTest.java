...
class TodoResourceTest extends BasicResourceTest {
    ...
    @Test
    void complete_withAlreadyCompletedTodo_shouldReturnAccepted() throws Exception {
        // Créez une tâche complétée (c'est-à-dire dont la propriété "completed" est à true).
    ...
    void create_withInvalidData_shouldReturnBadRequest() throws Exception {
        // Tester la création d'une tâche avec des données invalides (par exemple, un titre vide).
        TodoVM invalidVm = new TodoVM();
        mockMvc.perform(
                        post(UrlMapping.Todo.ADD)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(TestUtil.convertObjectToJsonBytes(invalidVm))
                )
                .andExpect(status().isBadRequest());
    }

    @Test
    void update_withNonExistingTodo_shouldReturnNotFound() throws Exception {
        // Tester la mise à jour d'une tâche qui n'existe pas.
        UUID nonExistingId = UUID.randomUUID();
        mockMvc.perform(
                        put(UrlMapping.Todo.UPDATE, nonExistingId)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(TestUtil.convertObjectToJsonBytes(vm))
                )
                .andExpect(status().isNotFound());
